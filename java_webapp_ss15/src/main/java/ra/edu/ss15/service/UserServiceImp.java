...
import ra.edu.ss15.model.dto.entity.RoleName;
...
import ra.edu.ss15.repository.RoleRepository;
import ra.edu.ss15.repository.UserRepository;
import ra.edu.ss15.security.jwt.JWTProvider;
import ra.edu.ss15.security.pricipal.CustomUserDetails;
...
public class UserServiceImp implements UserService {
    ...
    public void updateUserRole(Long id, Role newRole) {
        User user = userRepository.findById(Math.toIntExact(id))
                .orElseThrow(() -> new NoSuchElementException("Không tồn tại người dùng với ID: " + id));
        ...
        if (!roles.contains(newRole)) {
            ...
            log.info("Cập nhật role thành công cho người dùng: {}", user.getUsername());
        } else {
            log.warn("Role đã tồn tại trong danh sách roles của người dùng: {}", user.getUsername());
        }
    }
